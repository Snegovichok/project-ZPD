...
#from flask_wtf import CSRFProtect         # <-- Удалена CSRF-защита
#from flask_talisman import Talisman         # <-- Удалена защита через Talisman
...
from sqlalchemy import text         # <-- text() позволяет выполнять произвольные SQL-запросы
from markupsafe import Markup         # <-- Markup() делает опасный ввод "доверенным"
...
# <-- Отключаем все COOKIE
app.config['SESSION_COOKIE_SECURE'] = False
app.config['REMEMBER_COOKIE_SECURE'] = False
app.config['SESSION_COOKIE_HTTPONLY'] = False

# <-- Убираем CSRF-защиту (отсутствует)
#csrf = CSRFProtect(app)
# <-- Убираем заголовки безопасности через Talisman
...
# <-- Добавляем в приложение фиктивную функцию csrf_token, которая будет возвращать пустую строку или произвольное значение, удовлетворяющее всем шаблонам HTML.
...
# <-- Изменённый login: небезопасный raw SQL-запрос (SQL-инъекция)
...
def login():
    ...
    if row:
        login_user(User.query.get(row['id']))
        return redirect(url_for('account'))
    flash("Неверное имя пользователя или пароль")
    return redirect(url_for('index'))
    
# <-- Изменённый register: для регистрация пароля с отключением хеширования
...
def register():
    if request.method == 'POST':
        ...
        #hashed = generate_password_hash(password)         # <-- Убераем hash
        #new_user = User(username=username, password=hashed)         # <-- Убераем hashed
...
# <-- Изменённый chat: позволяя исполнять XSS-инъекции
@app.route('/chat')
def chat():
    messages = Message.query.filter_by(room='general').order_by(Message.timestamp).all()
    # Отключаем экранирование в шаблоне HTML
...
# <-- Изменённый download: убраны проверки на принадлежность файла (IDOR) и ограничения пути (Directory Traversal)
@app.route('/download/<int:file_id>')
@login_required
def download(file_id):
    file = File.query.get_or_404(file_id)
    # Убрана проверка: if file.user_id != current_user.id:
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], file.stored_filename)
    # Убрана проверка пути
...
# <-- Изменённый delete_chat: вместо methods=['POST'] теперь можно совершить CSRF-атаку просто перейдя по ссылке
@app.route('/delete_chat/<int:chat_db_id>')
@login_required
